...
if __name__ == '__main__':
  ...
  try: 
    ...
    print(' usage: $ python3 benchmark.py <agent-name> <back-move> (<timeout>) (<log>)')
  ...
  curr_dir = os.path.join(curr_dir,back_move)
  if not os.path.isdir(curr_dir): os.mkdir(curr_dir)
  ...
  tests = read_tests(os.path.join(in_dir,back_move))
  total = 0
  START = datetime.datetime.now()

  for test_file in tests:
    done = 0; print('---------',test_file)
